...
function CalculadoraPage() {
  ...
  const handleSubmitCalculo = useCallback(async (e) => { // Hacer async
    ...
    const menoresValidos = menores.map(m => ({ ...m, edad: parseInt(m.edad, 10), })).filter(m => !isNaN(m.edad) && m.edad >= 0 && m.edad <= 18);
    if (menoresValidos.length !== menores.length) { setErrorCalculo("Por favor, ingrese edades válidas (0-18) para todos los menores."); setIsLoading(false); return; }
        ...
        <div className="md:col-span-1">
          <SectionCard title="Calculadora de Prestaciones Sociales">
            <form onSubmit={handleSubmitCalculo}>
              <div className="mb-4"><label htmlFor="ingresosPadres" className="block text-sm font-medium text-gray-700 mb-1">Ingreso Nominal Mensual Conjunto de los Padres ($)</label><input type="number" id="ingresosPadres" value={ingresosPadres} onChange={(e) => setIngresosPadres(e.target.value)} placeholder="Ej: 50000" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" required /></div>
              <div className="mb-6"><label htmlFor="leyActual" className="block text-sm font-medium text-gray-700 mb-1">Prestación Actual</label><select id="leyActual" value={leyActual} onChange={(e) => setLeyActual(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"><option value="AFAM">Asignación Familiar (Ley 15.084)</option><option value="PE">Plan de Equidad (Ley 18.227)</option></select></div>
              <div className="mb-6">
                ...
                <Button type="button" variant="ghost" onClick={addMenor} className="mt-2 text-sm"><i className="fas fa-plus-circle mr-2"></i>Añadir Menor</Button>
              </div>
