/* ========================================================================== */
/* Archivo: frontend/src/pages/CalculadoraPage.jsx (LAYOUT 2 COLUMNAS)       */
/* ========================================================================== */
...
function CalculadoraPage() {
  ...
  const addMenor = () => {
    setMenores(prevMenores => [
      ...prevMenores,
      { id: uuidv4(), edad: '', esMultiple: false, nivelEducativo: 'Otro' },
    ]);
  };
  ...
  const handleSubmitCalculo = useCallback((e) => {
    e.preventDefault();
    setIsLoading(true);
    ...
    })).filter(m => !isNaN(m.edad) && m.edad >= 0 && m.edad <= 18); // Añadir validación de edad máxima
    ...
    if (menoresValidos.length === 0 && menores.length > 0) { // Si había menores pero ninguno fue válido
        setErrorCalculo("Por favor, ingrese edades válidas para los menores.");
        setIsLoading(false);
        return;
    }
     if (menores.length === 0) { // Si no se ingresó ningún menor
  ...
  const resetForm = () => {
    setIngresosPadres('');
    setLeyActual('AFAM');
    setMenores([{ id: uuidv4(), edad: '', esMultiple: false, nivelEducativo: 'Otro' }]);
    setCalculoResultados(null);
    setErrorCalculo(null);
    setIsLoading(false);
  };
    ...
    <main className="flex-grow container mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 md:gap-x-8 lg:gap-x-12">
        ...
        <div className="md:col-span-1">
          <SectionCard title="Comparador de Asignaciones Familiares" className="mb-6">
            <form onSubmit={handleSubmitCalculo}>
              ...
              <div className="mt-8 flex items-center space-x-3">
                <Button type="submit" variant="primary" disabled={isLoading}>
                  {isLoading ? (<><i className="fas fa-spinner fa-spin mr-2"></i>Calculando...</>) : (<><i className="fas fa-calculator mr-2"></i>Calcular Prestaciones</>)}
                </Button>
                <Button type="button" variant="ghost" onClick={resetForm} disabled={isLoading}>
                  <i className="fas fa-sync-alt mr-2"></i>Limpiar Formulario
                </Button>
              </div>
        ...
        <div className="md:col-span-1">
          {calculoResultados && !isLoading && !errorCalculo && (
            <SectionCard title="Resultados de la Simulación" className="mt-0 md:mt-0"> {/* Ajustar margen para la columna */}
              <div className="space-y-6">
                <div>
                  <h4 className="text-lg font-semibold text-secondary mb-2">
                    Escenario 1: Ley Actual ({calculoResultados.escenarioLeyActual.leyAplicada})
                  </h4>
                  <p className="text-2xl font-bold text-primary mb-2">
                    Monto Total: ${calculoResultados.escenarioLeyActual.montoTotal.toFixed(2)}
                  </p>
                  <details className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
                    <summary className="cursor-pointer font-medium hover:text-primary">Ver Desglose Ley Actual</summary>
                    <pre className="mt-2 whitespace-pre-wrap text-xs">{calculoResultados.escenarioLeyActual.desgloseCalculo}</pre>
                  </details>
                </div>

                <div>
                  <h4 className="text-lg font-semibold text-secondary mb-2">
                    Escenario 2: Con Ley de Nacimientos Múltiples (Ley 20.365)
                  </h4>
                  <p className="text-2xl font-bold text-primary mb-2">
                    Monto Total Combinado: ${calculoResultados.escenarioConNuevaLey.montoTotalCombinado.toFixed(2)}
                  </p>
                  <details className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md mb-2">
                    <summary className="cursor-pointer font-medium hover:text-primary">Ver Desglose Hijos Múltiples</summary>
                    <pre className="mt-2 whitespace-pre-wrap text-xs">{calculoResultados.escenarioConNuevaLey.desgloseCalculo.parteMultiples}</pre>
                  </details>
                  <details className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">
                    <summary className="cursor-pointer font-medium hover:text-primary">Ver Desglose Hermanos No Múltiples</summary>
                    <pre className="mt-2 whitespace-pre-wrap text-xs">{calculoResultados.escenarioConNuevaLey.desgloseCalculo.parteNoMultiples}</pre>
                  </details>
                </div>
                ...
                <div className="pt-4 border-t border-gray-200">
                  ...
                  <div className={`p-4 rounded-md text-center ${calculoResultados.comparacion.diferencia > 0 ? 'bg-green-50 border-green-200 border' : calculoResultados.comparacion.diferencia < 0 ? 'bg-orange-50 border-orange-200 border' : 'bg-blue-50 border-blue-200 border'}`}>
                    <p className={`text-md font-medium ${calculoResultados.comparacion.diferencia > 0 ? 'text-green-700' : calculoResultados.comparacion.diferencia < 0 ? 'text-orange-700' : 'text-blue-700'}`}>
                      {calculoResultados.comparacion.recomendacion}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {/* Placeholder si no hay resultados aún */}
          {!calculoResultados && !isLoading && !errorCalculo && (
             <div className="md:sticky md:top-24"> {/* Para que se mantenga visible al hacer scroll en el form */}
                <SectionCard title="Resultados de la Simulación" className="mt-0 md:mt-0">
                    <p className="text-gray-500 italic">Ingrese los datos en el formulario y presione "Calcular Prestaciones" para ver los resultados aquí.</p>
                </SectionCard>
             </div>
          )}
        </div>
